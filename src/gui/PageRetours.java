...
public class PageRetours extends JPanel implements IPage {
	...
	private JTextField tfPrenom, tfNom, tfId, tfTypeObjet, tfReference;
	...
	private void initialiserUI() {
		...
		c.gridx = 1;
		tfId = new JTextField();
		tfId.setPreferredSize(fieldSize);
		panelGauche.add(tfId, c);
	...
	private void traiterRetour() {
		Integer compteId;
		try {
			compteId = Integer.parseInt(tfId.getText().trim());
		} catch (NumberFormatException e) {
			labelResultat.setText("ID abonnement invalide");
			return;
		}
		...
		// Récupération client via compteId
		Integer clientIdFromCompte = clientDAO.getClientFromCompte(compteId);
